...
def _recalc_prices(item: Inventory, rate: float):
    """Пересчитать продажные цены по формуле: закупка * курс * (1 + наценка%)."""
    base_rub = item.purchase_price * rate
...
async def upload_inventory(
    ...
    """Загрузка остатков из Excel. Формат: артикул, [бренд], [описание], количество, цена закупки."""
    import pandas as pd
    ...
    # Пробуем определить колонки или берём по позиции
...
def record_transaction(
    part_number: str = Form(...),
    tx_type: str = Form(...),   # sale / return / adjust
    quantity: int = Form(...),
    price: float = Form(0),
    notes: str = Form(""),
    db: Session = Depends(get_db),
    ...
    if tx_type == "sale":
        if item.quantity < quantity:
            return {"error": f"Недостаточно на складе: {item.quantity} шт."}
        ...
        item.quantity = quantity  # Прямая установка
        qty_change = quantity
    else:
...
def recalc_all_prices(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """Пересчитать ВСЕ цены по текущему курсу."""
    rates = get_latest_rates(db)
...
def update_markups(
    ...
    """Обновить наценки для всех позиций и пересчитать цены."""
    rates = get_latest_rates(db)
    items = db.query(Inventory).all()
    for item in items:
...
def export_excel(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    headers = [
        ...
        "Закупка (USD)", "Курс USD", "Закупка (₽)",
        "Наценка заказ %", "Цена заказ ₽",
        "Наценка 3PL %", "Цена 3PL ₽",
        "Наценка 3PL+Emex %", "Цена 3PL+Emex ₽",
    ]
    ...
    for row_idx, item in enumerate(items, 2):
        cost_rub = round(item.purchase_price * usd_rate, 2) if usd_rate else 0
        ...
        ws.cell(row=row_idx, column=6, value=usd_rate)
        ws.cell(row=row_idx, column=7, value=cost_rub)
        ws.cell(row=row_idx, column=8, value=item.markup_order_pct)
        ws.cell(row=row_idx, column=9, value=item.price_order)
        ws.cell(row=row_idx, column=10, value=item.markup_3pl_pct)
        ws.cell(row=row_idx, column=11, value=item.price_3pl)
        ws.cell(row=row_idx, column=12, value=item.markup_3pl_emex_pct)
        ws.cell(row=row_idx, column=13, value=item.price_3pl_emex)
        for c in range(1, 14):
